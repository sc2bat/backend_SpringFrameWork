...
create table cart (
  ...
  id             varchar2(16)   references member(userid),   -- 주문자 아이디(FK :　member.id) 
...
create table orders(
  ...
  id          varchar2(16)   references member(userid), -- 주문자 아이디
...
select d.odseq, o.oseq, o.indate,  o.id, 
			m.name as mname, m.zip_num, m.address, m.address2, m.phone, 
...
-- 220307 
--ALTER TABLE member RENAME COLUMN address TO address1
ALTER TABLE member ADD address2 VARCHAR2(50);
...
SELECT cart_seq.nextVal, cart_seq.currVal FROM dual;

UPDATE order_detail SET result='2' WHERE oseq=26;


SELECT ROWNUM AS rn, p.* FROM product p ORDER BY pseq DESC;

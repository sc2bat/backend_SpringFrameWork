...
//	public ModelAndView category(/* HttpServletRequest request, Model model */) {
	...
	public ModelAndView category(@RequestParam("kind") String kind) {
		...
		// 동시에 저장하고 리턴하여 전달값과 이동페이지를 한번에 다룰수 있게하는 클래스입니다.
		
		// 매개변수 위치에 있는 @RequestParam("kind") String kind 는 변수 선언과 동시에 전달되는 파라미터를 받아서 저장하는 역할을 합니다
		// HttpServletRequest request 와 int kind = Integer.parseInt(request.getParameter("kind")); 는 생략이 가능합니다
		// 전달된 파라미터가 10개면 매개변수가 @RequestParam("") 형태로 10개 선언되어 사용됩니다.
		// int 형 변수도 선언이 가능합니다
		
		// 단점
		// 1. 문자데이터가 전달되는데, 정수형 변수를 사용하면 안됩니다
		// 2. 기본 특성상 null 값이 전달되면 오류가 발생합니다. null 값을 허용하려면 아래와 같이 명시해줍니다

...
public class BoardTestDao {
	...
	public ArrayList<BoardTestDto> getBoardMain(Paging paging) {
		ArrayList<BoardTestDto> boardlist = new ArrayList<BoardTestDto>();
		con = dbm.getConnection();
//		String sql = "SELECT * FROM springBoard ORDER BY num DESC";
		String sql = "SELECT * FROM ("
						+ " SELECT * FROM ( "
							+ "SELECT rownum AS rn, sb.* FROM (SELECT * FROM springBoard ORDER BY num DESC) sb"
						+ " ) WHERE rn >= ?"
				+ " ) WHERE rn <= ?";
		try {
	...
	public int replyCount(int num) {
		int rcnt = 0;
		String sql = "SELECT COUNT(*) AS cnt FROM springReply WHERE boardnum=?";
		con = dbm.getConnection();
		try {
			pstmt = con.prepareStatement(sql);
			pstmt.setInt(1, num);
	...
	public int getPageCount() {
		int count = 0;
		String sql = "SELECT COUNT(*) AS cnt FROM springBoard";
		con = dbm.getConnection();
		try {
			pstmt = con.prepareStatement(sql);
			rs = pstmt.executeQuery();
